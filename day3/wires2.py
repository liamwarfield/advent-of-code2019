...
def fill_board(board, step_board, wires):
    wire_num = 0
    for wire in wires:
        wire_num += 1
        num_steps = 0
            ...
            if step[0] == "U":
                for i in range(step[1]):
                    ...
                    if board[pos[0],pos[1]] == wire_num or board[pos[0],pos[1]] == 0:
                        ...
                        board[pos[0],pos[1]] = -2
                    step_board[pos[0],pos[1]] += wire_num
            elif step[0] == "D":
                for i in range(step[1]):
                    pos[1] -= 1
                    num_steps += 1
...
def search_for_X(board, step_board):
...
def main():
    ...
    step_board = np.zeros((22583,14404))
    print("Adding wires to the board")
    fill_board(board, step_board, wires)
    #printboard(board, step_board, 130)
    print("Performing Bruteforce search")
    min_dist = search_for_X(board, step_board)
